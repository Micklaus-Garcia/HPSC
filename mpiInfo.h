...
class mpiInfo
{
 public:
  ...
  int nei_ne, nei_nw, nei_se, nei_sw;   // <------ NEW
...
void PEsum(VD &field)
      ...
      err = MPI_Irecv(phiRecv_w, county, MPI_DOUBLE, nei_w, tag, MPI_COMM_WORLD, &request);   
          ...
          if (nei_n>=0) sLOOP field[pid( s        , nRealy+1 )] = phiT[s];
          if (nei_s>=0) sLOOP field[pid( s        , 0        )] = phiB[s];
          if (nei_e>=0) tLOOP field[pid( nRealx+ 1, t        )] = phiR[t];
          if (nei_w>=0) tLOOP field[pid( 0        , t        )] = phiL[t];
